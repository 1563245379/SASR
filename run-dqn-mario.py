...
def parse_args():
    ...
    parser.add_argument("--print-frequency", type=int, default=10000,
                        help="Print progress every N env steps")
    parser.add_argument("--write-frequency", type=int, default=1000,
                        help="Write losses to TensorBoard every N env steps")
    parser.add_argument("--save-frequency", type=int, default=500_000,
                        help="Save model every N env steps")
...
def main():
    ...
    pbar = tqdm(range(args.total_timesteps), desc="DQN-Mario Learning")
    ...
    for global_step in pbar:
        ...
        if len(memory) > args.learning_starts:
            ...
            if global_step % args.write_frequency == 0:
                writer.add_scalar("losses/q_loss", loss_val, global_step)
                writer.add_scalar("losses/q_value", q_val, global_step)

        # Print progress
        if global_step % args.print_frequency == 0:
            elapsed = time.perf_counter() - start_time
            fps = args.print_frequency / max(elapsed, 1e-6)
            avg_return = np.mean(recent_returns) if recent_returns else 0.0
            pbar.write(
                f"Step {global_step} | avg_return: {avg_return:.2f} | "
                f"train_steps: {global_train_steps} | buffer: {len(memory)} | "
                f"FPS: {fps:.0f}"
            )
            start_time = time.perf_counter()
        ...
        if global_step > 0 and global_step % args.save_frequency == 0:
            convert_and_save_sasr_format(
                ...
                args.exp_name, f"step{global_step}", args.seed
            )
