...
class SuperMarioBrosEnv(NESEnv):
    ...
    # MARK: Curriculum Learning

    def set_curriculum_position(self, target_x, render=False,
                                actions_dir='./curriculum_actions'):
        """Replay a recorded action sequence to reach target_x position.

        Loads pre-recorded actions from actions_dir/target_XXXX.npy and
        replays them to reach the target position deterministically.
        Creates a new backup at the reached position so that subsequent
        reset() calls restore Mario there.

        Args:
            target_x (int): the target horizontal position to reach
            render (bool): whether to render frames during replay
            actions_dir (str): directory containing recorded action .npy files

        """
        import os

        actions_path = os.path.join(actions_dir, 'target_{}.npy'.format(target_x))
        if not os.path.exists(actions_path):
            raise FileNotFoundError(
                "No recorded actions found at {}. "
                "Use play-mario.py --record {} to record first.".format(
                    actions_path, target_x)
            )

        actions = np.load(actions_path)
        print("Loading {} actions from {}".format(len(actions), actions_path))

        # Hard reset the emulator (bypass backup restore)
        self._has_backup = False
        self.reset()
        self._skip_start_screen()

        # Replay recorded actions with star invincibility
        for i, action in enumerate(actions):
            self.ram[0x079E] = 255  # star invincibility timer
            self._frame_advance(int(action))
            if render:
                self.render()

        # Clear invincibility and let state stabilize
        self.ram[0x079E] = 0
        for _ in range(30):
            self._frame_advance(0)
            if render:
        ...
        # Save current state as new checkpoint
        self._backup()
        self._time_last = self._time
        self._x_position_last = self._x_position
        print(
            "Curriculum position set: target_x={}, actual_x={} "
            "(replayed {} actions)".format(
                target_x, self._x_position, len(actions))
        )
